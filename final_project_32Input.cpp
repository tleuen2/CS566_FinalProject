...
int main(int argc, char *argv[])
{
    ...
    Path node_S_QE_ReceiveBuffer_missed[size];
    int node_S_flag[size];
    int node_S_flag_missed[size];
    ...
    if(size > 1){
        ...
        missedGuyRank = getMissedGuy(rank, size);
        ...
        while (!pq.empty()) // if there still exists some partial solution are not explored complete, keep check the result
        {
            ...
            for(int i=1; i<size; i++){
                ...
                node_S_QE_missed[i] = node_S_QE_missed[0];
            ...
            if(rank == myRepresentative && ((iter+1)%1)){
                if(MPI_Send(&node_S_QE_missed[missedGuyRank], 1, MPI_Path, missedGuyRank, 37, MPI_COMM_WORLD) == MPI_SUCCESS){
                    ...
                    printf("Error Sending data 37 to processor %d\n",missedGuyRank);
                    printf("ERROR - I am rank %d and I sent data to %d\n",rank, missedGuyRank);
                }
                int tempInt = missedGuyRank;
                //pq.push(missedGuysSthNode);
                MPI_Iprobe(missedGuyRank, 37, MPI_COMM_WORLD, &node_S_flag_missed[missedGuyRank], &status[missedGuyRank]);
                if(node_S_flag_missed[missedGuyRank] != 0){
                    ...
                    node_S_flag_missed[missedGuyRank] = 0;
                }
            }
            //printf("I am rank %d and step is %d\n",rank, 4);
...
int getRepresentative(int rank,int num_processor){
    ...
    //printf("I am rank %d and my representative is %d\n", rank, answer);
